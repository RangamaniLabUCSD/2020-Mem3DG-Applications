...
for i in range(nSubSize):
    ...
    l1bendnorm[i] = np.sum(abs(g.getLumpedMassMatrix() * g.getBendingPressure())) / g.surfaceArea 
    l1surfnorm[i]= np.sum(abs(g.getLumpedMassMatrix() * g.getCapillaryPressure())) / g.surfaceArea
    l1pressnorm[i]= np.sum(abs(g.getLumpedMassMatrix() * g.getInsidePressure())) / g.surfaceArea
    # l1linenorm[i]= np.sum(abs(g.getLineTensionPressure())) / g.surfaceArea
    # lineenergy[i]=g.E.lE
    bendenergy[i]=g.E.BE
    surfenergy[i]=g.E.sE
...
error=abs(l1bendnorm - l1bendnorm[nSubSize-1]
          )[:nSubSize-1] / abs(l1bendnorm[nSubSize-1])
popt, pcov=curve_fit(myExpFunc, lengthScale, error)
ax.loglog(lengthScale, error,
          '-o',  label="$f_b, {0:.1f}L^{{{1:.1f}}}$".format(*popt))

error=abs(l1surfnorm - l1surfnorm[nSubSize-1]
          )[:nSubSize-1] / abs(l1surfnorm[nSubSize-1])
popt, pcov=curve_fit(myExpFunc, lengthScale, error)
ax.loglog(lengthScale, error,
          '-o', label="$f_s, {0:.1f}L^{{{1:.1f}}}$".format(*popt))

error=abs(l1pressnorm - l1pressnorm[nSubSize-1]
          )[:nSubSize-1] / abs(l1pressnorm[nSubSize-1])
popt, pcov=curve_fit(myExpFunc, lengthScale, error)
ax.loglog(lengthScale, error,
...
error=abs(bendenergy - bendenergy[nSubSize-1]
          )[:nSubSize-1] / abs(bendenergy[nSubSize-1])
print(error)
popt, pcov=curve_fit(myExpFunc, lengthScale, error)
ax.loglog(lengthScale, error,
          '-o', label="$E_b, {0:.1f}L^{{{1:.1f}}}$".format(*popt))

error=abs(surfenergy - surfenergy[nSubSize-1]
          )[:nSubSize-1] / abs(surfenergy[nSubSize-1])
print(error)
popt, pcov=curve_fit(myExpFunc, lengthScale, error)
ax.loglog(lengthScale, error,
          '--', label="$E_s, {0:.1f}L^{{{1:.1f}}}$".format(*popt))

error=abs(pressenergy - pressenergy[nSubSize-1]
          )[:nSubSize-1] / abs(pressenergy[nSubSize-1])
popt, pcov=curve_fit(myExpFunc, lengthScale, error)
ax.loglog(lengthScale, error,
